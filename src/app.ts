...
async function dovizCevirici() {
    ...
    try{
        ...
        const dovizRes = await fetch(`https://open.er-api.com/v6/latest/${anaBirim === 'BTC' ? 'USD' : anaBirim}`).then(data => data.json()) as any;
        const btcRes = await fetch("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd,eur,try").then(data => data.json())as any;
